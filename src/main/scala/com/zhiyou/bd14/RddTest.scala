...
object RddTest {
  ...
  //重新分区
  var filePartitionNum = 0
  var resultPartitionNum = 0
  var coalescePartitionNum = 0
  ...
  def repartitionTest(): Unit = {
    ...
    file.foreachPartition(x => {
      filePartitionNum += 1
      println(s"fileRDD $filePartitionNum 分区, 该分区数据${x.size}条")
    })

    result.foreachPartition(x => {
      var sum = 0
      x.foreach(x => sum += 1)
      resultPartitionNum += 1
      println(s"resultRDD $resultPartitionNum 分区, 该分区数据: $sum 条")
    })

    //coalesce分区
    val coalResult = result.coalesce(3)
    coalResult.foreachPartition(x => {
      coalescePartitionNum += 1
      println(s"coalResultRDD $coalescePartitionNum 分区, 该分区数据: ${x.size} 条")
    })
  }
  ...
  def groupBy(): Unit = {
    ...
    groupedBy.foreach(x => {
      println(s"groupedByRDD的一条记录, key为${x._1}," +
        s" value上集合的记录条数是: ${x._2.size}条")
    })
  }
  ...
  def aggSumTest(): Unit = {
    ...
    val aggregateResult = rdd.aggregate("")(
      (c, v) => {
        c match {
          case "" => v.toString
          case _ => s"$c, $v"
  ...
  def persistTest() = {
    ...
    val userInIpAmount = file
      .filter(x => {
        x.contains("login")
      })
      .map(x => (x.split("\\t")(0) + x.split("\\t")(2), x))
      .distinct().count()
